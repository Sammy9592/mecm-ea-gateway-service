...
@Slf4j
public class SEATokenHandlerGatewayFilterFactory extends AbstractGatewayFilterFactory<AuthCerts> {

    @Autowired
    private TokenService mecmTokenService;

    public SEATokenHandlerGatewayFilterFactory() {
    ...
    public GatewayFilter apply(AuthCerts entitlement) {
        return (exchange, chain) -> Mono.just(exchange)
                    ...
                    return mecmTokenService.getSessionTrustToken(entitlement.getSource(), sessionToken)
                            ...
                            .doOnNext(theResponseObject -> {
                                ...
                                if (!ErrorCode.SUCCESS.getCode().equals(code) || !StringUtils.hasText(theResponseObject.getString(CommonVariables.DATA))){
                                    log.error("error to retrieve session trust token:" + theResponseObject);
                                    throw new MECMServiceException(ErrorCode.NO_AUTH.getCode(),
                                            "failed to retrieve session trust token:" + theResponseObject.getString(CommonVariables.MESSAGE),
                                            theResponseObject);
                                }
                            })
                ...
                .onErrorResume(e -> {
                    log.warn("failed to check out session trust token:" + e.getLocalizedMessage());
